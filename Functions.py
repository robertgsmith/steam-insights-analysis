...
Description: Stores all functions used in the 'main.py' file
...
def stat_histogram(df,column):
    '''
    Creates and shows the histogram of provided data based on provided column
    Parameters:
        df: pandas dataframe
        column: str, column which should be used for drawing histogram
    '''
...
def stat_simple_plot(df, col):
    '''
    Creates and shows the simple graph of provided data based on provided column
    Parameters:
        df: pandas dataframe
        col: str, column which should be used for drawing histogram
    '''
...
def parse_price(j):
    '''
    Parses a JSON-formatted price string and extracts key pricing information into a dictionary.
    :param j: needed to apply function row-vise
    Disclaimer: generated by ChatGPT version GPT5
    '''
    if pd.isna(j):
        return {}
    try:
...
def clean_languages(raw):
    '''
    Cleans and parses a raw string of language information (possibly containing HTML tags and special markers) into two lists:
    :param raw: needed to apply function row-vise
    Disclaimer: generated by ChatGPT version GPT5
    '''
    if pd.isna(raw):
        return [], []
...
def time_helper(text:str):
    '''
    Internal tracker of the time needed to perform the action
    :param text:
    :return:
    '''
...
def kpi_owner_acquisition_rate(df):
    '''
    Compute the KPI "Owner Acquisition Rate" for publishers.
...
def kpi_positive_review_share(final_data):
    '''
    Compute the KPI "% of Positive reviews" for publishers.
    :param final_data: pandas DataFrame
    '''
...
def kpi_monitisation_efficiency(df):
    '''
    Compute the KPI "Efficiency of Monetisation" for publishers.
    :param df: pandas DataFrame
    '''
...
def kpi_engagement_ratio(df):
    '''
    Compute the KPI "% Engagement rate" for publishers.
    :param df: pandas DataFrame
    '''
